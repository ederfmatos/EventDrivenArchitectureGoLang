...
func (handler *TestEventHandler) Handle(event events.Event) {
	handler.CallsCount++
}

func MakeSut() (*DefaultEventDispatcher, *TestEventHandler, TestEvent) {
	return NewDefaultEventDispatcher(), NewTestEventHandler(1), TestEvent{Name: "EventName", Payload: "EventPayload"}
}
...
func Test_Should_Register_An_Event_Dispatcher(t *testing.T) {
	...
	err = dispatcher.Register(event.Name, handler)
	assert.ErrorIs(t, err, ErrorHandlerAlreadyRegistered, "Error registering event handler")
